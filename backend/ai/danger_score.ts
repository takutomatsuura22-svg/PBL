...
export function calculateDangerScore(factors: StudentRiskFactors): number {
  ...
  // 1. モチベーションが低い（重み: 30%）
  // モチベーションが低いほど危険度が高い
  const motivationRisk = (6 - factors.motivation_score) / 5 * 5 // 1-5スケールに変換
  score += motivationRisk * 0.3
  weightSum += 0.3

  // 2. 負荷が高い（重み: 25%）
  // 負荷が高いほど危険度が高い
  const loadRisk = factors.load_score // 既に1-5スケール
  score += loadRisk * 0.25
  weightSum += 0.25
 ...
 * 危険度スコアの詳細内訳を取得
 */
export function getDangerScoreBreakdown(factors: StudentRiskFactors): {
  motivation: number
  load: number
  overdue: number
  skillGap: number
  ...
  return {
    motivation: (6 - factors.motivation_score) / 5 * 5,
    load: factors.load_score,
    overdue: Math.min(5, factors.overdue_tasks * 1.5),
    skillGap: factors.skill_gap * 5,
    activity: (1 - factors.recent_activity) * 5,
    communication: factors.communication_gap * 5
  }
}
